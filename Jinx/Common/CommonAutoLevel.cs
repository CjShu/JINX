...
namespace Jinx.Common
{
    internal class CommonAutoLevel
    {
        ...
        public static void Init(Menu parentMenu)
        {
            ...
            if (string.Equals(ObjectManager.Player.ChampionName, "Jinx", StringComparison.InvariantCultureIgnoreCase))
            {
                SpellLevels = new[] {1, 3, 2, 1, 1, 4, 1, 2, 1, 2, 4, 2, 2, 3, 3, 4, 3, 3};
                MenuLocal.AddItem(new MenuItem("AutoLevel." + ObjectManager.Player.ChampionName,
                    GetLevelList(SpellLevels)));
            }
