            ...
            MenuLocal = new Menu("Drawings", "Drawings");
            {
                ...
                SubMenuSpells = new Menu("Spell Ranges", "DrawSpellRanges");
                {
                    for (int i = 0; i < 2; i++)
                    {
                        ...
                        SubMenuSpells.AddItem(new MenuItem(pcMode[i] + "Draw.W", "E:").SetValue(new Circle(false, Color.FromArgb(255, 255, 255, 255))).SetFontStyle(FontStyle.Regular, E.MenuColor()));
                ...
                SubMenuAxe = new Menu("Axe Times", "DrawAxeTimers");
                {
                    for (int i = 0; i < 2; i++)
                    {
                        SubMenuAxe.AddItem(new MenuItem(pcMode[i] + "Draw.AxePosition", "JinxAxe Position").SetValue(new StringList(new[] { "Off", "Circle", "Line", "Both" }, 3)).SetFontStyle(FontStyle.Regular, E.MenuColor()));
                        SubMenuAxe.AddItem(new MenuItem(pcMode[i] + "Draw.AxeTime", "JinxAxe Time Remaining").SetValue(true)).SetFontStyle(FontStyle.Regular, E.MenuColor());
                    }
                    MenuLocal.AddSubMenu(SubMenuAxe);
                }

                MenuLocal.SubMenu("Drawings").AddItem(new MenuItem("Draw.AxePosition", "JinxAxe Position").SetValue(new StringList(new[] { "Off", "Circle", "Line", "Both" }, 3)));
                MenuLocal.SubMenu("Drawings").AddItem(new MenuItem("Draw.AxeTime", "JinxAxe Time Remaining").SetValue(true));
